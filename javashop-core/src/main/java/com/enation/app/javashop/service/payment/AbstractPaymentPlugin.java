...
public abstract class AbstractPaymentPlugin {
    ...
    protected String getReturnUrl(PayBill bill) {
        ...
        if (ClientType.UNIAPP.name().equals(client)) {
            ...
            String redirectUri =  "/#/pages/cashier/cashier?";

            StringBuffer url = new StringBuffer(domainHelper.getMobileDomain());
//            StringBuffer url = new StringBuffer("http://192.168.2.233:8081");

            url.append(redirectUri);

            if (TradeTypeEnum.TRADE.name().equals(tradeType)) {
                url.append("trade_sn=" + bill.getSubSn());
            } else {
                url.append("order_sn=" + bill.getSubSn());
            }

            url.append("&is_callback=yes&default_plugin_id="+bill.getPluginId());
            String result = "";
            try {
            ...
            return domainHelper.getCallback() + "/payment/return/" + tradeType + "/" + payMode + "/" + this.getPluginId();
        }
    }
