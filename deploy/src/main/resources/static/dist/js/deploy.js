...
function executeDeploy(type,deployid) {
    var tips = type == "redis" ? "清空" : "部署";
    if ( !confirm("本操作会" +tips+ "所有的数据，确认要执行"+tips+"吗？") ){
        return false;
    }
 ...
 * @param deployid
 */
function executeDelete(deployid) {

    if ( !confirm("本操作会清除所有数据库中的示例数据，确认要执行吗？") ){
        return false;
    }
    ...
    $.ajax({
        url: "/data/databases/"+deployid+"/delete",
        type: "POST",
        success: function (data) {
            if (data == "ok") {
        ...
        columns: [ //定义列
            ...
            }  ,
            {
                ...
                "render": function (data, type, row) {
                    ...
                    btnHtml+='<div class="btn-group deploy_btn_box" >'
                    btnHtml+='    <button type="button" class="btn btn-info">清除</button>'
                    btnHtml+='    <button type="button" class="btn btn-info dropdown-toggle" data-toggle="dropdown" aria-expanded="false">'
                    btnHtml+='        <span class="caret"></span>'
                    btnHtml+='        <span class="sr-only">Toggle Dropdown</span>'
                    btnHtml+='    </button>'
                    btnHtml+='    <ul class="dropdown-menu" role="menu">'
                    btnHtml+='        <li><a href="javascript:executeDelete('+data["deploy_id"]+')">清除示例数据</a></li>'
                    btnHtml+='        <li><a href="javascript:executeDeploy(\'redis\','+data["deploy_id"]+')" >清空Redis</a></li>'
                    btnHtml+='    </ul>';
                    btnHtml+='</div>';
                    return btnHtml;
                }
